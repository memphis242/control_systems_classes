...
%% Observer
des_poles_H = [-3,-4,-5,-6];
H = transpose(placePoles(Aapprox',Capprox',Capprox,des_poles_H));
Ae = Aapprox; Be = Bapprox; Ce = Capprox; De = D;

%% Linear Simulation
% U = Kr*R - Kx*X
Aaug = [Ae, -Be*Kx; H*Ce, Ae-H*Ce-Be*Kx]; Baug = [Be;Be]*Kr;
Caug = [Ce, Ce*0; 0*Ce, Ce]; Daug = [0;0];
t = transpose(linspace(0,15,1001)); R = 0*t + 1;
X0 = [zeros(4,1); rand(4,1)];
Y = step3(Aaug,Baug,Caug,Daug, t,X0,R);
plot(t,Y,'LineWidth',2);
legend('y(t)','y_{est}(t)');
grid on; title('Linear Simulation: y vs y_{est}'); xlabel('Time (s)');
pause;

%% Nonlinear Simulation
clf
V = zeros(20,1); Z = 0; Xe = zeros(4,1);
...
%     Z = Z + dZ * dt;
    ...
    DATA(i,:) = [V([5,10,15,20])', Xe', V0, V(20)];
    i = i+1;
...
subplot(1,2,1);
plot(t,DATA(:,[1:4]), t,DATA(:,[5:8]), '--');
grid on;
title('Simulated Response of Heat System: State vs Observer State'); xlabel('Time (s)');

subplot(1,2,2);
plot(t,DATA(:,9), t,DATA(:,10),'LineWidth',2);
legend('u(t)','y(t)');
grid on;
title('Output of System'); xlabel('Time (s)');
